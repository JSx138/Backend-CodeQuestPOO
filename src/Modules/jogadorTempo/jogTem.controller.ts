...
export async function getHistoricoSemanal(req: Request, res: Response) {
    try {
        const aluno = Number(req.alunoId!)
        ...
        const historico = await JogadorTempoService.getHistoricoSemanal(aluno)
        return res.status(200).json(historico)
    } catch (error) {
      res.status(400).json("Erro ao buscar histórico semanal do jogador")
    }
}
